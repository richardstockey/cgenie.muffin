...
# 20250703 I am now adding some new features to this to make a version of runmuffin-basket.sh doesnt need a base config for each user config
# now have an argument 4 and 5. 
# 4 is a stop pattern (e.g., "Yohler_b") and 5 is a suffix (e.g., ".BASES")
...
# Assume you pass two extra arguments to your script:
...
printf "(cd /scratch/$USER/cgenie.muffin-$i/genie-main; make cleanall; LD_LIBRARY_PATH=/scratch/rgs1e22/cgenie.muffin-$i/lib; export LD_LIBRARY_PATH; ./runmuffin.sh $base_name $2/$line ${line}-${iteration}.config $3 &> ~/cgenie_log/muffin-basket-$(date '+%F_%H.%M')-${line}-${iteration}.log) &
"  >> ~/cgenie.jobs/muffin-basket-$short_name-$iteration.sbatch
else # wait 6 mins (recall five occassionally not being quite enough) for second 20 runs
j=$((i-20))
printf "(cd /scratch/$USER/cgenie.muffin-$j/genie-main; sleep 360; make cleanall; LD_LIBRARY_PATH=/scratch/rgs1e22/cgenie.muffin-$j/lib; export LD_LIBRARY_PATH; ./runmuffin.sh $base_name $2/$line ${line}-${iteration}.config $3 &> ~/cgenie_log/muffin-basket-$(date '+%F_%H.%M')-${line}-${iteration}.log) &
"  >> ~/cgenie.jobs/muffin-basket-$short_name-$iteration.sbatch
fi
else # subsequent experiments all start from a restart
printf "(cd /scratch/$USER/cgenie.muffin-$i/genie-main; make cleanall; LD_LIBRARY_PATH=/scratch/rgs1e22/cgenie.muffin-$i/lib; export LD_LIBRARY_PATH; ./runmuffin.sh $base_name $2/$line ${line}-${iteration}.config $3 ${line}-$((iteration - 1)).config &> ~/cgenie_log/muffin-basket-$(date '+%F_%H.%M')-${line}-${iteration}.log) &
"  >> ~/cgenie.jobs/muffin-basket-$short_name-$iteration.sbatch
fi
i=$((i+1))
done
